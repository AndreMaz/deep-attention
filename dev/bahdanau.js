...
let encoderGRUOutput = tf.layers
  .bidirectional({
    layer: tf.layers.gru({ units: lstmUnits, returnSequences: true })
  })
  .apply(encoderEmbeddingOutput);

console.log(`Shape of encoderGRUOutput`);
console.log(encoderGRUOutput.shape);
encoderGRUOutput.print();
...
decoderGRUOutput = tf.layers
  .gru({ units: lstmUnits * 2, returnSequences: true })
  .apply(decoderEmbeddingOutput, { initialState: [encoderLast] });

console.log(`Shape of decoderLTSMOutput`);
console.log(decoderGRUOutput.shape);
decoderGRUOutput.print();
