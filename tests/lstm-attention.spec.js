...
const { expectArraysClose } = require("@tensorflow/tfjs-core/dist/test_util");
...
describe("Model", () => {
  it("Created model can train", async () => {
    ...
    const decoderOutputs = tf.randomUniform([
      numExamples,
      outputLength,
      outputVocabSize
    ]);
  ...
  it("Model save-load roundtrip", async () => {
    ...
    const modelPrime = await tf.loadLayersModel(
      `file://${saveDir.name}/model.json`
    );
